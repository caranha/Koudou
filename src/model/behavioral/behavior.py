...
class Behavior:
	...
	def step(self,kd_sim,kd_map,step_count,step_length,rng,agent,logger):
		move_action_pool = []
		for act in self.activities:
			if act.check_conditions(agent,kd_sim, kd_map, step_count,rng):
				if agent.previous_activity != act.name:
					temp_activity = {}
					temp_activity["time"] = ts.get_hour_min_str(step_count)
					temp_activity["time_stamp"] = step_count
				...
				actions = act.generate_actions(agent,kd_map,step_count,rng)
